...
# Set up sensor
def init_sensor(pixformat=sensor.RGB565, framesize=sensor.HQVGA, 
windowsize=None,
                gain=18, autoexposure=False, exposure=5000, 
autowhitebal=False,
...
def color_detection(data):
    ...
    blobs = img.find_blobs([ballon_threshold], merge=True, 
area_threshold=25, pixels_threshold=10)
    if blobs:
        red_led.on()
        max_blob = find_max(blobs)

        return struct.pack("<HHHH", max_blob.cx(), max_blob.cy(), 
max_blob.w(), max_blob.h())
    else:
        red_led.off()
...
if __name__ == "__main__":
    ...
    ballon_threshold = (30,75, 40,70,-40,20)
    init_sensor(pixformat=sensor.RGB565, framesize=sensor.QQVGA, 
windowsize=None,
                gain=6, autoexposure=False, exposure=10000, 
autowhitebal=False,
